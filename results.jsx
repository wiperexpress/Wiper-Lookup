...
function SkuPill({ sku, muted }) {
  ...
  React.useEffect(() => {
    if (!window.__SKU_MAP_LOADED) {
      const h = () => force();
      window.addEventListener('sku-map-loaded', h);
      return () => window.removeEventListener('sku-map-loaded', h);
    }
  }, []);
  const link = typeof window !== 'undefined' && window.shopifyUrlFor ? window.shopifyUrlFor(sku) : null;
  const pillStyle = {
    ...
    border: link && !link.mapped ? `1px dashed ${C.blueDark}33` : '1px solid transparent',
  };
  if (!link) {
    return <span style={pillStyle}>{sku}</span>;
  }
  const title = link.mapped
    ? `View ${sku} on Shopify`
    : `Search Shopify for ${sku} (not yet mapped)`;
  return (
    <a
      href={link.url}
      target="_blank"
      ...
      onMouseEnter={(e) => { e.currentTarget.style.background = muted ? '#E9E9E9' : '#C7D9FF'; e.currentTarget.style.boxShadow = `0 0 0 2px ${C.blue}22`; }}
      onMouseLeave={(e) => { e.currentTarget.style.background = muted ? C.surface : C.blueLight; e.currentTarget.style.boxShadow = 'none'; }}
      style={pillStyle}
    >
      <span>{sku}</span>
      <svg width="9" height="9" viewBox="0 0 10 10" style={{ opacity: link.mapped ? 0.75 : 0.45, flexShrink: 0 }}>
        <path d="M3 1 H9 V7 M9 1 L3 7" stroke="currentColor" strokeWidth="1.4" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </a>
  );
}
          ...
          <div>
            ...
            {motorSku ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                {(() => {
                  const link = typeof window !== 'undefined' && window.shopifyUrlFor ? window.shopifyUrlFor(motorSku) : null;
                  const body = (
                    <div style={{
                      ...
                      textDecoration: 'none',
                      border: link && !link.mapped ? `1px dashed ${C.blueDark}55` : `1px solid transparent`,
                    }}>
                      {motorSku}
                      {link && <svg width="13" height="13" viewBox="0 0 10 10" style={{ opacity: link.mapped ? 0.75 : 0.5 }}><path d="M3 1 H9 V7 M9 1 L3 7" stroke="currentColor" strokeWidth="1.4" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                  );
                  return link ? (
                    <a href={link.url} target="_blank" rel="noopener noreferrer"
                       title={link.mapped ? `View ${motorSku} on Shopify` : `Search Shopify for ${motorSku}`}
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {r.amk ? (() => {
              const link = typeof window !== 'undefined' && window.shopifyUrlFor ? window.shopifyUrlFor(r.amk) : null;
              const badge = (
                <div style={{
                  ...
                  {r.amk}
                  {link && (
                    <svg width="12" height="12" viewBox="0 0 10 10" style={{ opacity: link.mapped ? 0.8 : 0.5 }}>
                      <path d="M3 1 H9 V7 M9 1 L3 7" stroke="currentColor" strokeWidth="1.4" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  )}
                </div>
                ...
                <>
                  {link ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      title={link.mapped ? `View ${r.amk} on Shopify` : `Search Shopify for ${r.amk}`}
...
Object.assign(window, { AutoResults, HdResults, RvResults, UtvResults, EmptyState, VehicleHeader, Stat, SkuPill });
